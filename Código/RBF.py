...
class Solver():
    ...
    def __init__(self, matrix, algorithm = None, tol = None , max_iter = None, test = None):
        '''
        Class Cosntructor.
        ...
        Param:
            matrix: GrammMatric class object instance.\n
            algorithm: String used to select linear system solution method, linalg, gmres, bcgstab, bicg, cg.\n
            tol: desired tolerance for the convergence of the selected algorithm.\n
            max_iter: Maximum number of iterations for the convergence of the selected algorithm.\n
            test: flag to inicate algorithms data return.\n
        '''
...
#----------------------------------------------------------------------------------------
    ...
    def solve(self):
        ...
        if self.__algorithm == 'linalg':
            ...
            self.__lam,self.__err,self.__k,self.__errv = sol.jacobi(G,f,self.__tol,self.__max_iter,self.__test)
        elif self.__algorithm == 'gs':
            self.__lam,self.__err,self.__k,self.__errv = sol.gauss_seidel(G,f,self.__tol,self.__max_iter,self.__test)
        elif self.__algorithm == 'sor':
            self.__lam,self.__err,self.__k,self.__errv = sol.sor(G,f,self.__tol,self.__max_iter,1.5,self.__test)
        elif self.__algorithm == 'cgm':
            b = np.matrix(f.copy())
            self.__lam,self.__err,self.__k,self.__errv = sol.cgm(G,b.T,b.T,self.__tol,self.__max_iter,self.__test)
        elif self.__algorithm == 'bicgstab':
            b = np.matrix(f.copy())
            self.__lam,self.__err,self.__k,self.__errv = sol.bicgstab(G,b.T,b.T,self.__tol,self.__max_iter,self.__test)
        elif self.__algorithm == 'gmres':
            b = np.matrix(f.copy())
            self.__lam,self.__err,self.__k,self.__errv = sol.gmres(G,b.T,b.T,self.__tol,self.__max_iter,15,self.__test)
           # print(self.lam())
